test_that("fails with invalid 'column_name'", {
  check_is_datetime(column_name = c("col_a", "col_b")) |>
    expect_error(
      stringr::str_c("Argument 'column_name' is invalid.",
                     " Must have length 1, but has length 2.")
    )

  check_is_datetime(column_name = factor("col_a")) |>
    expect_error(
      stringr::str_c("Argument 'column_name' is invalid.",
                     " Must be of type 'character', not 'factor'.")
    )

  # test if the column_name parameter is missing
  check_is_datetime(column_name = "") |>
    expect_error(
      stringr::str_c("Argument 'column_name' is invalid.",
  ...
  # test integer
  check_is_datetime(column_name = 1L) |>
    expect_error(
      stringr::str_c("Argument 'column_name' is invalid.",
                     " Must be of type 'character', not 'integer'.")
    )

  # test double
  check_is_datetime(column_name = 1.1) |>
    expect_error(
      stringr::str_c("Argument 'column_name' is invalid.",
  ...
  # test logical
  check_is_datetime(column_name = TRUE) |>
    expect_error(
      stringr::str_c("Argument 'column_name' is invalid.",
                     " Must be of type 'character', not 'logical'.")
    )
})

test_that("returned function fails with something other than ADaM dataframe", {
  check_is_datetime(column_name = "col_a")() |>
...
test_that("returned function provides correct 'msg' for invalid columns", {
  expect_checker_false(
    ...
    checker = check_is_datetime(column_name = "col_a"),
    msg = stringr::str_c("Column 'col_a' must have data type 'datetime',",
...
test_that("a message is returned if the column doesn't exist", {
  expect_checker_false(
    data = dplyr::tibble(col_a = as.POSIXct("2020-01-01 22:22:22")),
    checker = check_is_datetime(column_name = "col_b"),
    msg = stringr::str_c("Column 'col_b' must have data type 'datetime',",
                         " but is data type 'NULL'.")
  )

  dplyr::tibble(col_a = as.POSIXct(NULL)) |>
...
test_that("the function returns 'TRUE' with valid column", {
  ...
  expect_checker_true(
    data = dplyr::tibble(col_a = c(as.POSIXct("2020-01-01 22:22:22"),
                                   as.POSIXct("2021-01-01 22:22:22"))),
    checker = check_is_datetime(column_name = "col_a")
  )
})
...
test_that("an appropriate message is returned for 1 invalid factor datatype", {
  expect_checker_false(
    ...
    checker = check_is_datetime(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'datetime', but is data type 'factor'."
  )
})
...
test_that("return 'FALSE' for 2 invalid datatypes", {
  ...
  dplyr::tibble(col_a = c(test_sex, test_datetime)) |>
    check_is_datetime(column_name = "col_a")() |>
    expect_false()
...
test_that("an appropriate message is returned for 2 invalid datatypes", {
  ...
  expect_checker_false(
    data = dplyr::tibble(col_a = c(test_date, test_date)),
    checker = check_is_datetime(column_name = "col_a"),
    msg = stringr::str_c("Column 'col_a' must have data type 'datetime',",
                         " but is data type 'Date'.")
  )
...
test_that("return 'TRUE' for 1 valid date/datetime datatypes", {
...
test_that("return 'TRUE' for 2 valid date/datetime datatypes", {
  dplyr::tibble(col_a = test_datetime, test_datetime) |>
    check_is_datetime(column_name = "col_a")() |>
    expect_true()
