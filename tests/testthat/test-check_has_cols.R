...
test_that("returns 'FALSE' if a tibble doesn't have specified columns", {
  dplyr::tibble(col_a = 1) |>
    check_has_cols(column_names = "col_b")() |>
    expect_false()
  ...
  dplyr::tibble(col_a = 1, col_b = 2) |>
    check_has_cols(column_names = c("col_c", "col_b"))() |>
    expect_false()

  dplyr::tibble(col_a = 1, col_b = 2, col_c = 3) |>
    check_has_cols(column_names = c("col_d", "col_c"))() |>
    expect_false()
})
...
test_that("'msg' describes which columns are missing when check fails", {
  ...
  expect_checker_false(
    ...
    checker = check_has_cols(column_names = "col_b"),
    msg = "Dataset must have columns 'col_b', but is missing 'col_b'."
  )

  expect_checker_false(
    data = dplyr::tibble(col_a = 1),
    checker = check_has_cols(column_names = c("col_a", "col_b", "col_c")),
    msg = stringr::str_c("Dataset must have columns 'col_a', 'col_b', 'col_c',",
                " but is missing 'col_b', 'col_c'.")
  )
  ...
  expect_checker_false(
    ...
    checker = check_has_cols(column_names = c("col_a", "col_b", "col_c")),
    msg = stringr::str_c("Dataset must have columns 'col_a', 'col_b', 'col_c',",
                " but is missing 'col_a', 'col_b', 'col_c'.")
  )
})

test_that("returns 'TRUE' if a tibble has specified columns", {
  dplyr::tibble(col_a = 1) |>
    check_has_cols(column_names = "col_a")() |>
    expect_true()
...
test_that("'msg' attribute is 'NULL' when check passes", {
  ...
  expect_checker_true(
    ...
    check_has_cols(column_names = c("col_a", "col_b", "col_3"))
  )
