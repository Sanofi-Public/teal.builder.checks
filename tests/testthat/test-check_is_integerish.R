...
test_that("returned function fails with something other than ADaM dataframe", {
  check_is_integerish(column_name = "col_a")() |>
    expect_error('argument "adam" is missing, with no default')

  dplyr::tibble(col_a = numeric()) |>
    check_is_integerish(column_name = "col_a")() |>
    expect_error(
...
test_that("returned function provides correct 'msg' for invalid columns", {
  expect_checker_false(
    ...
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'numeric'."
  )

  expect_checker_false(
    data = dplyr::tibble(col_a = "2020-01-01"),
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'character'."
  )

  dplyr::tibble(col_a = as.integer(NULL)) |>
...
test_that("a message is returned if the column doesn't exist", {
  expect_checker_false(
    ...
    checker = check_is_integerish(column_name = "col_b"),
    msg = "Column 'col_b' must have data type 'integer' or 'whole number double', but is data type 'NULL'."
  )
})

test_that("the function returns 'TRUE' with valid column", {
  expect_checker_true(
    data = dplyr::tibble(col_a = as.integer(1.1)),
    checker = check_is_integerish(column_name = "col_a")
  )

  expect_checker_true(
    data = dplyr::tibble(col_a = c(as.integer(2.2), as.integer(3.6))),
    checker = check_is_integerish(column_name = "col_a")
  )
})

test_that("return 'FALSE' for 1 invalid numeric datatype", {
...
test_that("return 'FALSE' for 1 invalid text datatype", {
  dplyr::tibble(col_a = "text") |>
    check_is_integerish(column_name = "col_a")() |>
    expect_false()
})

test_that("return 'FALSE' for 1 invalid factor datatype", {
...
test_that("an appropriate message is returned for 1 invalid text datatype", {
  expect_checker_false(
...
test_that("an appropriate message is returned for 1 invalid factor datatype", {
  expect_checker_false(
    data = dplyr::tibble(col_a = test_sex),
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'factor'."
  )
})
...
test_that("an appropriate message is returned for 1 invalid logical datatype", {
  expect_checker_false(
    ...
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'logical'."
  )
...
test_that("an appropriate message is returned for 1 invalid date datatype", {
  expect_checker_false(
    ...
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'Date'."
  )

  expect_checker_false(
    data = dplyr::tibble(col_a = as.POSIXct("2020-01-01 00:00:00")),
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'POSIXct', 'POSIXt'."
  )
})

test_that("an appropriate message is returned for 1 invalid numeric datatype", {
  expect_checker_false(
...
test_that("return 'FALSE' for 2 invalid datatypes", {
  ...
  dplyr::tibble(col_a = test_double, test_double) |>
    check_is_integerish(column_name = "col_a")() |>
    expect_false()

  dplyr::tibble(col_a = test_date, test_date) |>
    check_is_integerish(column_name = "col_a")() |>
    expect_false()

  dplyr::tibble(col_a = c(test_date, test_datetime)) |>
    check_is_integerish(column_name = "col_a")() |>
    expect_false()
...
test_that("an appropriate message is returned for 2 invalid datatypes", {
  expect_checker_false(
    data = dplyr::tibble(col_a = c(test_sex, test_sex)),
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'factor'."
  )
  ...
  expect_checker_false(
    data = dplyr::tibble(col_a = c(test_date, test_datetime)),
    checker = check_is_integerish(column_name = "col_a"),
    msg = "Column 'col_a' must have data type 'integer' or 'whole number double', but is data type 'Date'."
  )
})
...
test_that("return 'TRUE' for 1 valid double datatypes", {
  dplyr::tibble(col_a = 1.0) |>
    check_is_integerish(column_name = "col_a")() |>
    expect_true()
})

test_that("return 'TRUE' for 1 valid NA datatype", {
...
test_that("return 'TRUE' for 1 valid number datatype", {
  ...
  dplyr::tibble(col_a = as.double(1)) |>
    check_is_integerish(column_name = "col_a")() |>
    expect_true()
})

test_that("no message is returned for 1 valid number datatype", {
  expect_checker_true(
  ...
  expect_checker_true(
    data = dplyr::tibble(col_a = as.double(1)),
    checker = check_is_integerish(column_name = "col_a")
  )
})

test_that("no message is returned for 1 valid date datatype", {
  expect_checker_true(
...
test_that("return 'TRUE' for 2 valid datatypes", {
  ...
  dplyr::tibble(col_a = c(as.double(1), as.double(2))) |>
    check_is_integerish(column_name = "col_a")() |>
    expect_true()
})
...
test_that("no message is returned for 2 valid datatypes", {
  ...
  expect_checker_true(
    data = dplyr::tibble(col_a = c(as.double(1), as.double(2))),
    checker = check_is_integerish(column_name = "col_a")
  )
